$(document).ready(function(){
	...
	$('.remove-cart').click(function(){
		...
		$.ajax({
        	...
        	if(data[0]=="success"){
        		this_element.parent().children().first().remove();
		...
		if($(this).hasClass('cart') && !$('#project-'+$(this).attr('id')).hasClass('done')){
			...
			$.ajax({
	        	...
	        	if(data[0] == "success"){
	        		...
	        		if($('#no-elements')){
	        			$('#no-elements').remove();
	        			$('#modal-cart-body').append(add_cart_element(data[2],data[3]));
	        			var tag = document.createElement("script");
						tag.src = "../../js/shop-page.js";
						document.getElementsByTagName("head")[0].appendChild(tag);
	        		}
	        		...
	        		$('.alerts').children().remove();
	        		$('.alerts').append('<div class="alert alert-danger" style="text-align: -webkit-center;" role="alert"> You have already added the project to your cart or you have bought it </div>');
	        	}else{
			...
			$.ajax({
	        	...
	        	if(data[0] == "success"){
	        		...
	        		$('#login-list').replaceWith('<li id= "li-cart"><p class="navbar-text"><a href="#" data-toggle="modal" data-target="#cart-modal">Cart <span class="badge" id="cart-count">'+data[1]+'</span> </a></p></li>'+'<li id = "li-logout"><p class="navbar-text"><a href = "#" id = "logout-link" ><span class = "glyphicon glyphicon-log-out"> logout </span> </a></p></li>');
	        		$('#modal-cart-body').children().remove();
	        		if(data[1]>0){
	        			for(var i = 0; i<data[2].length;  i++){
	        				$('#modal-cart-body').append(add_cart_element(data[2][i]['cart_id'],data[2][i]['name']));
	        			}
	        		}else{
		...
		$.get( "../../actions/list_projects.php?university_name="+university_name+"&course_name="+course_name, function( data ) {
			for(var i = 0; i<data.length; i++){
				if(data[i]['id'] == null){
					...
					var modal2 = "<div id='project-"+data[i]['id']+"' class='modal fade' tabindex='-1' role='dialog' aria-hidden='true'><div class='modal-dialog'><div class='modal-content'><div class='modal-header'><button type='button' class='close' data-dismiss='modal' aria-hidden='true'>×</button><h1 class='text-center'>"+data[i]['name']+"</h1></div><div class='modal-body'><span class='pull-right'><a href='#'>$ "+data[i]['price']+"</a></span><span><a href='#'>"+data[i]['count']+" Available</a></span><h3 class='form col-md-12 center-block text-decore'>"+data[i]['description']+"</h3><form  method='post' remote= 'true' class='cart' id="+data[i]['id']+"><input type='hidden' name='project_id' value="+data[i]['id']+"><input type='submit' name = 'submit' class='btn btn-primary btn-lg btn-block' value='Add To Cart'></form></div><div class='modal-footer'><div class='col-md-12'></div></div></div></div></div>"
					if(data[i]['image_link'] == null){
				 		$('.projects').append("<div class='col-sm-4 col-lg-4 col-md-4'><div class='thumbnail'><img src='http://placehold.it/320x150' alt=''><div class='caption'><h4 class='pull-right'> $"+data[i]['price']+"</h4><h4><a href='#' data-toggle='modal' data-target='#project-"+data[i]['id']+"'>"+data[i]['name'].substring(0,12)+"..."+"</a></h4><p>"+data[i]['description'].substring(0,63)+"..."+"</p></div><div class='ratings'><p class='pull-right'>"+data[i]['count'] +" Availabe</p><p><span class='glyphicon glyphicon-star'></span><span class='glyphicon glyphicon-star'></span><span class='glyphicon glyphicon-star'></span><span class='glyphicon glyphicon-star'></span><span class='glyphicon glyphicon-star'></span></p></div></div></div>"+modal2);
				 	}else{
				 		$('.projects').append("<div class='col-sm-4 col-lg-4 col-md-4'><div class='thumbnail'><img src='"+data[i]['image_link']+"' alt=''><div class='caption'><h4 class='pull-right'> $"+data[i]['price']+"</h4><h4><a href='#'data-toggle='modal' data-target='#project-"+data[i]['id']+"'>"+data[i]['name'].substring(0,12)+"..."+"</a></h4><p>"+data[i]['description'].substring(0,63)+"..."+"</p></div><div class='ratings'><p class='pull-right'>"+data[i]['count'] +" Availabe</p><p><span class='glyphicon glyphicon-star'></span><span class='glyphicon glyphicon-star'></span><span class='glyphicon glyphicon-star'></span><span class='glyphicon glyphicon-star'></span><span class='glyphicon glyphicon-star'></span></p></div></div></div>"+modal2);
	...
	function add_cart_element(cart_id,cart_name){
		// var test = $('<button/>',
		//     {
		//         text: 'Test',
		//         click: function () { alert('hi'); }
		//     });
		// $("<li></li>").class("list-group-item").id("cart-"+cart_id).text(cart_name).children(); 
		return '<li class="list-group-item" id ="cart-'+cart_id+'">'+cart_name+'<div class="btn-group pull-right" style="margin-top: -1%;"><button class="btn btn-success btn-sm buy-cart">BUY</button><button class="btn btn-danger btn-sm remove-cart">REMOVE</button></div></li>';
	}
});